...
> **実装時の訂正(v1):** 上の JSON 例は `ArmLeft` / `ArmRight` に同じ `arm_standard_01` を
> 割り当てているが、§2.2 の `PartDefinition.slot` は単一スロットなので、腕は左右で別アセットになる。
> 実装では `arm_standard_l_01` / `arm_standard_r_01` のように左右別 ID を保存する。
> 左右は別パーツ、という前提で確定。
